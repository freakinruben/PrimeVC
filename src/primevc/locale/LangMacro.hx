...
class LangMacro 
{
    ...
    {      
		...
		for (yaml in langsRaw) 
			for (lang in yaml.elements)	// ILang interface generation. Also creates Typedefs.
	...
	static private function traverseXMLGenerateExpValues(xml:Fast, parent:String = "")
	{
		...
		for (el in xml.elements)
			switch( getElementType(el) )
			{
				case leaf:	result +=  "this.bindables." + (parent.length > 0? parent + "." : "") + el.name + ".value = this.current." + (parent.length > 0?parent + "." : "") +el.name +";";
				case node:	result += traverseXMLGenerateExpValues(el, (parent.length > 0? parent + "." : "")  + el.name);
				case func:
				case plural:
			}
		return result;
	...
	static private inline function isLeaf(xml:Fast)			return !xml.elements.hasNext()
	static private inline function isPlural(xml:Fast)		return xml.has.plural
	static private inline function isFunction(xml:Fast)		return xml.has.func
